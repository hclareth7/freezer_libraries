...
class FreezerScheduler(object):
    ...
    def poll(self):
        ...
        for job_doc in work_job_doc_list:
            if self.client.client_id == job_doc['client_id']:
                job_id = job_doc['job_id']
                if (('schedule_interval' not in job_doc['job_schedule'] or job_doc['job_schedule']['schedule_interval'] == '') and
                        'status' in job_doc['job_schedule'] and job_doc['job_schedule']['status'] == 'completed'):
                    LOG.debug("Job {0} was scheduled for one-time only. Skipping".format(job_id))
                else:
